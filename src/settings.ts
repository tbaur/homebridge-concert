 ...
 */
export const MAX_ACCESSORY_TEXT_LENGTH = 64
 ...
 */
export const STATE_QUERY_RETRIES = 1

/** Delay before retrying a timed-out / closed state query. */
export const STATE_QUERY_RETRY_MS = 500
 ...
 */
export const QUERY_BREAKER_FAILURE_THRESHOLD = 3

/** First cool-off once the query breaker opens; doubles per further failure. */
export const QUERY_BREAKER_BASE_COOLDOWN_MS = 30_000

/** Ceiling for the query breaker cool-off. */
export const QUERY_BREAKER_MAX_COOLDOWN_MS = 600_000
 ...
 */
export const STATE_FRESHNESS_MS = 5_000
 ...
 */
export const POWER_SET_TIMEOUT_MS = HOMEKIT_WRITE_BUDGET_MS
 ...
 */
export const POWER_STATE_TTL_MS = 5 * 60_000
 ...
 */
export const WAKE_RETRY_TIMEOUT_MS = 60_000
 ...
 */
export function readPluginVersion(log?: { warn?: (message: string) => void }): string {
  ...
  try {
    ...
    log?.warn?.(
      `Could not read plugin version from package.json (${describeError(error)}); `
      + `reporting ${UNKNOWN_PLUGIN_VERSION} as FirmwareRevision.`,
    )
    cachedPluginVersion = UNKNOWN_PLUGIN_VERSION
  }
  return cachedPluginVersion
}
