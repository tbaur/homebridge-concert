...
function createPlatform(source = 'cd') {
  const onChar = {
...
describe('SourcePresetAccessory', () => {
  ...
  it('reports On only when polled source matches the target', async () => {
    const { platform, accessory, switchService } = createPlatform()
    const client = mockClient({
      getSource: jest.fn()
        .mockResolvedValueOnce('bd')
  ...
  it('keeps last known On when source poll fails after being On', async () => {
    ...
    expect(platform.log.warn).toHaveBeenCalledWith(
      'XR-8S CD: poll failed: standby',
    )
    const getHandler = onChar.onGet.mock.calls[0][0] as () => boolean
    expect(getHandler()).toBe(true)
  })

  it('skips source poll when the zone is last known standby', async () => {
    ...
    expect(client.getSource).not.toHaveBeenCalled()
    expect(platform.log.debug).toHaveBeenCalledWith(
      'XR-8S CD: skipping source poll (zone in standby)',
    )
  })
  ...
  it('requires source in context', () => {
    const { platform, accessory } = createPlatform()
    ;(accessory.context as { source?: string }).source = undefined
    const client = mockClient()

    expect(() => new SourcePresetAccessory(platform, accessory, client)).toThrow(/missing source/)
  })

  it('rejects unknown source in context', () => {
    const { platform, accessory } = createPlatform('tape')
    expect(() => new SourcePresetAccessory(platform, accessory, mockClient())).toThrow(
      /unknown source/,
    )
  })
  ...
  it('single-flights concurrent refresh calls', async () => {
    ...
    expect(client.getSource).toHaveBeenCalledTimes(1)
    resolvePoll?.('cd')
  ...
  it('logs recovery after a successful poll following failures', async () => {
    ...
    expect(platform.log.debug).toHaveBeenCalledWith(
      'XR-8S CD: poll failed: timeout',
    )
    expect(platform.log.info).toHaveBeenCalledWith('XR-8S CD: poll recovered')
  })
  ...
  it('skips poll ticks while a source set is in flight', async () => {
    const { platform, accessory, onChar } = createPlatform()
    let resolveSet: (() => void) | undefined
    const client = mockClient({
      setSourceWhenReady: jest.fn().mockImplementation(() => new Promise<void>((resolve) => {
